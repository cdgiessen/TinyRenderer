...
vec3 Model::vert(const int i) const { return verts_[i]; }
...
{
    return diffusemap_.get(static_cast<int>(uvf[0] * diffusemap_.get_width()),
                           static_cast<int>(uvf[1] * diffusemap_.get_height()));
}

vec3 Model::normal(const vec2 &uvf) const
{
    TGAColor c = normalmap_.get(static_cast<int>(uvf[0] * normalmap_.get_width()),
...
{
    return specularmap_.get(static_cast<int>(uvf[0] * specularmap_.get_width()),
                            static_cast<int>(uvf[1] * specularmap_.get_height()))[0];
}
